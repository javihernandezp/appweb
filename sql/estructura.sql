...
CREATE TABLE `tienda`.`pedidos` ( `id` INT(11) NOT NULL AUTO_INCREMENT, `emailcliente` VARCHAR(25) NOT NULL, `idproducto` INT(11) NOT NULL , `nombre_producto` VARCHAR(25) NOT NULL,`fechapedido` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (`id`,`emailcliente`,`nombre_producto`),FOREIGN KEY (emailcliente) REFERENCES clientes (email) ON DELETE CASCADE ON UPDATE CASCADE, FOREIGN KEY (nombre_producto) REFERENCES productos (nombre) ON DELETE CASCADE ON UPDATE CASCADE);
